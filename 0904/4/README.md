...
* **현실적인 대안들 (The Alternatives):**
    ...
    2.  **DTO + DAO 통합 (e.g., `sqlmodel`):**
        FastAPI 생태계의 `sqlmodel` 라이브러리가 이 접근의 좋은 예시입니다. 애플리케이션 입출력 명세(DTO)와 DB 스키마(DAO)를 **하나의 클래스**로 정의하게 해줍니다.
...
* **명확한 책임 분리 (Clarity):**
    버그가 발생했을 때, 이것이 '잘못된 입력 데이터(DTO)'의 문제인지, '비즈니스 규칙(Entity)'의 문제인지, 'DB 저장(DAO)'의 문제인지 명확히 알 수 있습니다.
