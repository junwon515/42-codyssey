...
* **엔티티 (Entity):**
    ...
    * 단순한 데이터 덩어리가 아닙니다. `src/domain/entity.py`의 `Todo` 엔티티는 스스로의 상태를 변경하는 `complete()`, `uncomplete()` 같은 '행위'를 포함합니다.
* **애그리거트 (Aggregate):**
...
### B. DTO - Entity - ORM Model의 명확한 분리

데이터 객체가 각 계층에서 다른 '역할'을 갖도록 명확히 분리했습니다.

1.  **DTO (Data Transfer Object):**
    * `src/application/dtos.py`
    * **역할:** 계층 간 데이터 교환을 위한 '표준 운송 컨테이너'이자, 애플리케이션 서비스의 '입출력 계약(Contract)' 입니다.
2.  **Entity (Domain Model):**
    * `src/domain/entity.py`
    * **역할:** '왕과 왕비'. 순수한 비즈니스 '행위'를 가진 핵심 객체입니다.
3.  **ORM Model (Persistence Model):**
    * `src/infrastructure/adapters_out/datebase/models.py`
...
### A. 고민 1: DTO-Entity-ORM Model 분리 (생산성 vs. 결합도)

* **문제:** DTO, 도메인 Entity, 그리고 ORM Model를 모두 분리하자, 이들 간의 데이터를 변환해주는 `mappers.py` 같은 '보일러플레이트(단순 반복)' 코드가 엄청나게 늘어났습니다.
...
* **현실적인 대안들 (The Alternatives):**
    ...
    1.  **Entity + ORM Model 통합 (ActiveRecord 패턴):**
        모델 객체(Entity)가 스스로 DB 저장/조회(ORM Model) 기능까지 갖는 방식입니다. (e.g., `user.save()`)
        * **단점:** 도메인 모델(Entity)이 특정 데이터베이스 기술(ORM Model)에 **강하게 결합**됩니다. '성의 중심부(도메인)'가 '외부(DB)'를 알게 되므로, 헥사고날 아키텍처의 원칙이 깨집니다.

    2.  **DTO + ORM Model 통합 (e.g., `sqlmodel`):**
        FastAPI 생태계의 `sqlmodel` 라이브러리가 이 접근의 좋은 예시입니다. 애플리케이션 입출력 명세(DTO)와 DB 스키마(ORM Model)를 **하나의 클래스**로 정의하게 해줍니다.
        * **단점:** 애플리케이션 계약(애플리케이션 계층)와 DB 스키마(데이터 계층)가 **결합**됩니다. 서비스의 응답 스펙를 바꾸는 것이 의도치 않게 DB 스키마에 영향을 줄 수 있습니다.

    3.  **DTO + Entity + ORM Model 모두 통합:**
...
* **이유:**
    ...
    * **애플리케이션 명세(DTO)의 변경:** "응답 데이터(Contract)에서 `creator_ip`를 마스킹해서 보여주세요."
    * **도메인(Entity)의 변경:** "Todo를 `complete`할 때, 특정 비즈니스 규칙을 추가해 주세요."
    * **데이터 계층(ORM Model)의 변경:** "DB 성능을 위해 `TodoTable`의 인덱스를 수정해 주세요."
...
* **명확한 책임 분리 (Clarity):**
    버그가 발생했을 때, 이것이 '잘못된 입력 데이터(DTO)'의 문제인지, '비즈니스 규칙(Entity)'의 문제인지, 'DB 저장(ORM Model)'의 문제인지 명확히 알 수 있습니다.
...
* **보일러플레이트 코드 증가 (Boilerplate):**
    가장 큰 단점입니다. 간단한 기능 하나를 추가하기 위해 `DTO`, `Entity`, `ORM Model`, `Mapper`, `Port`, `Adapter` 등 많은 파일을 만들어야 합니다.
